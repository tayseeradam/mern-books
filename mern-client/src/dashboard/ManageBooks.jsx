...
const ManageBooks = () => {
  ...
  useEffect(() => {
    fetch("BACKEND_URL")  // Updated URL
      //https://book-store-3bg5.onrender.com/
      .then(res => {
        if (!res.ok) {
  ...
  return (
    <div className='px-4 my-12'>
      <h2 className='mb-8 text-3xl font-bold font-serif'>Manage Your Books</h2>
      {/* table for book data */}
      <Table className='lg:w-[1180px]'>
        <Table.Head>
          ...
          <Table.HeadCell>
            <span className="sr-only font-serif">Edit or Manage</span>
          </Table.HeadCell>
        </Table.Head>
        {
              ...
              <Table.Row className="bg-yellow-300 dark:border-gray-700 dark:bg-gray-800">
                ...
                <Table.Cell>
                  <Link to={`/admin/dashboard/edit-books/${book._id}`} 
                    className="font-medium text-cyan-600 hover:underline dark:text-cyan-500 mr-5 font-serif">
                    Edit
                  </Link>
